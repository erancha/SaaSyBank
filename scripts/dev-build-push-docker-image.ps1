...
try {
   ...
   # Tag and push image
   $formattedElapsedTime = Get-ElapsedTimeFormatted -startTime $startTime
   Write-Host "`n$(Get-Date -Format 'HH:mm:ss'), elapsed $formattedElapsedTime : Tagging and pushing image..."
   docker tag "${lowerServiceName}:${imageTag}" "$ecrImageUri"
   docker push "${ecrUri}/${ecrRepositoryName}:${imageTag}"

   $formattedElapsedTime = Get-ElapsedTimeFormatted -startTime $startTime
   Write-Host "`n$(Get-Date -Format 'HH:mm:ss'), elapsed $formattedElapsedTime : Completed. Image pushed successfully to ECR."
